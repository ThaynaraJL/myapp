 ...
 * 22.06.2010
 */
 var coordenadasDoPoligono = new Array();
...
function Pen(map){
    ...
    this.draw=function(latLng){
        if(null!=this.polygon){
            alert('A seleção já foi concluída! Você pode refazer a seleção após clicar em "Limpar seleção" nas opções à esquerda.');
        }
        else{
            if(this.currentDot!=null && this.listOfDots.length>1 && this.currentDot==this.listOfDots[0]){
                ...
                coordenadasDoPoligono.push(latLng);
    ...
    this.drawPloygon=function(listOfDots,color,des,id){
        ...
        insereCoordenadasNoInput(coordenadasDoPoligono);
    }
...
function Dot(latLng,map,pen){
    ...
    this.addListener=function(){
        ...
        var thisDot=this;google.maps.event.addListener(thisMarker,'click',function(){
...
function Polygon(listOfDots,map,pen,color){
    ...
    var thisObj=this;$.each(this.listOfDots,function(index,value){
        thisObj.coords.push(value.getLatLng());
    });

    this.polygonObj=new google.maps.Polygon({paths:this.coords,
        strokeColor:'white',
        strokeOpacity:0.5,
        strokeWeight:2,
        fillColor:'white',
        fillOpacity:0.2,map:this.map
    });
